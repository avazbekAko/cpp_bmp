...
unsigned char* read_file(const char* filename){
    ...
    if (FileHeaderSize != sizeof(BMPFileHeader)){
        fclose(SourceImage);
        cout << "BMPFileHeader was read wrong (size issue)!\n";
        return 0;
    }
    cout << FileHeaderSize << " = size of header\n";
    ...
    if (BytesRead != (3 * width + padding) * height){
        delete[] PixelInfo;
        fclose(SourceImage);
        cout << BytesRead << " - bytes read, " << (3 * width + padding) * height << " bytes should be\n";
        cout << "Pixel information read wrongly\n";
        return 0;
    }
    fclose(SourceImage);
    return PixelInfo;
...
unsigned char* write_file(const char* filename, BMPFileHeader FileHeader, unsigned char* data){
    ...
    if (FileHeaderSize != sizeof(BMPFileHeader)){
        fclose(ImageWrite);
        cout << "Header is written wrongly\n";
        return 0;
    }
    ...
    if (WrittenBytes != (3 * FileHeader.width + padding) * FileHeader.height){
        cout << WrittenBytes << " / " << (3 * FileHeader.width + padding) * FileHeader.height << "bytes written\n";
        fclose(ImageWrite);
    }
    else {
...
int main() {
	...
	unsigned char* PixelInfo = read_file("640X426.bmp");
	cout << fileheader.file_size << " bytes = file size\n";
	...
	write_file("Result-TurnedLeft.bmp", LeftRHeader, LeftRData);
	delete[] LeftRData;
	...
	write_file("Result-TurnedRight.bmp", RightRHeader, RightRData);
	delete[] RightRData;

	BMPFileHeader GausHeader = fileheader;
	unsigned char* GausData = gaussian_blur(GausHeader, PixelInfo);
	write_file("Result-GaussianFilter.bmp", fileheader, GausData);
	delete[] GausData;
